...
| Adapter RDP — worker (spike) | `docs/adr/0006` | AUSENTE | **contrato congelado no PR-17B0** (docs-only): C++20 + API C do FreeRDP 3.28.0 fixado, `rdp-worker/` isolado, UDS-only, build nativo no CI; código ainda não escrito; **validação real no P0** (Windows/xrdp) | PR-17B (implementação, após merge do 17B0); depois smoke P0 (fora do sandbox) |
