...
| Adapter RDP — worker (spike) | `rdp-worker/` | PARCIAL | **PR-17B**: worker C++20 sobre `libfreerdp` (boundary `freerdp_client`), UDS `0600`+peer creds, SecureBuffer, redação, allowlist, lifecycle. **Build lógico + 8 suites de unit test verdes localmente** (`-Werror`); **binding nativo FreeRDP 3.28.0 e P0 pendentes de CI/host** (sandbox sem `docker pull`/repo FreeRDP) | native build via CI `rdp-worker-build-test`; depois smoke P0 (Windows/xrdp) |
