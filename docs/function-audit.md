...
| Adapter RDP — worker (spike) | `rdp-worker/` | PARCIAL | **PR-17B**: worker C++20 sobre `libfreerdp` (boundary `freerdp_client`), UDS `0600`+peer creds, SecureBuffer, redação, allowlist, lifecycle. **CI `rdp-worker-build-test` verde**: build lógico + 8/8 unit tests (`-Werror`), **build nativo + 8/8 testes nativos + `--selftest` confirmou FreeRDP 3.28.0**, imagem runtime executou, SBOM gerado+validado. **Só falta a prova ponta-a-ponta contra alvo real (P0)** | smoke P0 (Windows NLA/xrdp), fora do CI comum |
